...
$$
\sigma_{SMD} = \sqrt{\frac{1}{\tilde n} \cdot \frac{N - 2}{N - 4} \cdot (1 + \tilde n \cdot
                                                             d ^ 2) - \frac{d^2}{J^2}}
$$
...
size (Cohen's d(av)), and the standard deviation of the control condition 
...
Therefore, I created the `smd_calc` function.
The interface is almost the same as `t_TOST` but you just don't set a equivalence bound.
